...
import { VoterAccount, voterAccountFromList } from "../../utils/types";
...
const VoterList = () => {
  ...
  const [voterList, setVoterList] = React.useState<VoterAccount[] | null>([]);
  ...
  useEffect(() => {
    ...
    (contracts.voterReader?.methods as any)
      ...
      .call()
      .then((res: any) => {
        var regs: VoterAccount[] = res.map(
          (voter: any) => voterAccountFromList(voter) ?? null
        );
        console.log(regs);
        setVoterList(regs);
      })
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold">Voters List</h1>
      <div className="overflow-x-auto mt-10">
        <p className="text-wrap text-lg mb-10">
          Here is the list of all voters who is registered and verified
        </p>
